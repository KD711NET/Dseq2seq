...
%%
%dep有关全部为正
%m=8099,表示EW3潜标位置最近的数据
wm2 = 1/(12.420601*3600);
ws2=1/(12*3600);
wk1=1/(23.9345*3600);
wo1=1/(25.8193*3600);
wn2=1/(12.6583*3600);
...
fileNamesWithoutExtension2 = cell(size(filenames2));
% 循环遍历 filenames2 中的每个文件名，并去掉后缀保存到新的 cell 组中
for i = 1:numel(filenames2)
    [~, fileName, ~] = fileparts(filenames2{i});
    fileNamesWithoutExtension2{i} = fileName;
end
...
for i = 1:length(mall)
    i
    ux = squeeze(uxs(i,:,slice2));
    vx = squeeze(vxs(i,:,slice2));
    saltx = squeeze(saltxs(i,:,slice2));
    tempx = squeeze(tempxs(i,:,slice2));
    ux2=redata2(ux);
    vx2=redata2(vx);
    tempx2=redata2(tempx);
    saltx2=redata2(saltx);
    ...
    if ~exist(folder_path, 'dir')
        mkdir(folder_path);
        disp(['文件夹已创建: ', folder_path]);
    else
        disp(['文件夹已存在: ', folder_path]);
    end
    ...
    if isempty(ux2) 
        ...
        %深度积分准备
            ...
            for ii = 1:m2
               ...
               top=dep0(1);
               u_barotropic(ii) = h_integral(ux2(:,ii),dep0,top,bottom)/(bottom-top);
               %v_barotropic(ii) = h_integral(vx2(:,ii),dep0,top,bottom)/(bottom-top);
               u_baroclinic(:,ii) = ux2(:,ii) - u_barotropic(ii);
               %v_baroclinic(:,ii) = vx2(:,ii) - v_barotropic(ii);
            end
            %内潮信号提取
            fc=1/(3*3600);
            [b,a]=butter(4,[0.85,1.06]*wk1/(fc/2),'bandpass');
            ...
            % 将 datetime 类型转换为指定格式的字符数组
            dateStrings = datestr(dateTimes, 'yyyy/mm/dd HH:MM');
            dateCellArray = cellstr(dateStrings);
      
            % 将 rowNames 和 data 组合成一个新的 cell 数组
            outputData = [{'date'},{'OT'};dateCellArray, num2cell(data')];
