...
def create_inout_sequences(input_data, tw, pre_len, config):
    """
    Create input-output sequences for time series data using sliding window approach

    Args:
        input_data: Full time series data (tensor)
        tw: Training window size (input sequence length)
        pre_len: Prediction length (output sequence length)
    ...
    for i in range(L - tw):
        ...
        # Check if we have enough data for the output sequence
        if (i + tw + pre_len) > len(input_data):
            break

        # Create output sequence differently based on feature mode
        if config.feature == 'MS':
            train_label = input_data[:, -1:][i + tw:i + tw + pre_len]  # Last column only for MS mode
        else:
            train_label = input_data[i + tw:i + tw + pre_len]  # All features for other modes
...
def create_empty_dataset():
    """
    Create an empty dataset placeholder
...
def create_dataloader3(config, device, filename):
    """
    Create DataLoader for time series data with proper preprocessing
    ...
    Returns:
        test_loader: DataLoader for the processed data
    ...
    train_data_normalized = scaler.transform(true_data)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--other_path', type=str, default='./datatestall/*', help='Root path containing data files')
    parser.add_argument('-window_size', type=int, default=96, help="Input sequence length (must be > pre_len)")
    parser.add_argument('-pre_len', type=int, default=24, help="Forecast horizon length")
    ...
    parser.add_argument('-input_size', type=int, default=1, help='Number of input features (excluding time column)')
    ...
    parser.add_argument('-drop_out', type=float, default=0.05, help="Dropout rate for regularization")
    ...
    parser.add_argument('-hidden_size', type=int, default=128, help="Number of hidden units in LSTM layers")
    parser.add_argument('-laryer_num', type=int, default=2, help="Number of LSTM layers")

    # Hardware configuration
    parser.add_argument('-use_gpu', type=bool, default=True, help="Enable GPU acceleration")
    parser.add_argument('-device', type=int, default=0, help="GPU device index (single GPU only)")

    # Runtime options
    parser.add_argument('-train', type=bool, default=True, help="Enable training phase")
    parser.add_argument('-test', type=bool, default=True, help="Enable testing phase")
    parser.add_argument('-predict', type=bool, default=True, help="Enable prediction phase")
    parser.add_argument('-inspect_fit', type=bool, default=True, help="Enable model fitting inspection")
    parser.add_argument('-lr-scheduler', type=bool, default=True, help="Enable learning rate scheduler")

    args = parser.parse_args()

    # Configure computing device (GPU if available and enabled)
    ...
    # Determine output size based on feature mode
    if args.feature == 'MS' or args.feature == 'S':
        args.output_size = 1  # Single output feature
    else:
        args.output_size = args.input_size  # Multiple output features
    ...
    try:
        ...
        print(
            f">>>>>>>>>>>>>>>>>>>>>>>>> Failed to initialize {args.model} model: {str(e)} <<<<<<<<<<<<<<<<<<<<<<<<<<<")
    import scipy.io
    # Configure data paths
    args.root_path = './datatestall/'
    args.data_path = 'diurdep28.csv'
    args.other_path = './datatestall/*'

    # Load geographical coordinate data
    lon = scipy.io.loadmat('lon.mat')
    lat = scipy.io.loadmat('lat.mat')
    ...
    # Process each data file with progress bar
    for file_path in tqdm(file_paths, desc="Processing data files"):
        ...
        try:
            ...
            if os.path.exists(full_path) and os.path.getsize(full_path) == 0:
                mse_in = 0  # Default MSE for empty files
            else:
                # Create test data loader and evaluate model
                ...
                lons.append(lon)
                lats.append(lat)
                mse_in = metrics_data['seq2seq'][1]  # Extract MSE value

        except Exception as e:
            print(f"Error processing {file_name}: {str(e)}")
            mse_in = 0  # Default MSE for failed cases

        mse_i_all.append(mse_in)

    # Convert MSE results to numpy array and reshape
    mse_i_array = np.array(mse_i_all)
    mse_i_2d = mse_i_array.reshape(97, 73, order='F')
    ...
    scipy.io.savemat('mse_Dseq_diur_dep0.mat', mat_dict)
